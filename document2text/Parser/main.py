...
from src._parser import MultiFormatParser
import argparse
from concurrent.futures import ProcessPoolExecutor
os.environ["CUDA_VISIBLE_DEVICES"] = "1" 

TXT_PATHS_DIR = r"output/paths"
FILE_ROOT = r""
SAVE_ROOT = r"output/txt_file"
model_path = r"./models/v8_layout.pt"
error_log = r"output/error.txt"
...
def process_file(file_path, model_path):
    '''
        处理单个文件
    '''
    try:
        parser = MultiFormatParser(model_path)
...
def inference(mode):
    ...
    paths = []
    with open(os.path.join(TXT_PATHS_DIR, str(mode) + ".txt"), 'r', encoding='utf-8') as f:
        for line in f:
            line = line.strip()
    ...
    for file_path in tqdm.tqdm(paths):
        try:
            process_file(file_path, model_path)
        except Exception as e:
            with open(error_log, 'a') as error:
                error.write(f"{file_path} Error: {str(e)}\n")
                error.flush()
