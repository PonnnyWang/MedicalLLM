...
hf_logging.set_verbosity_info()  # Set the logging level to INFO
...
def train():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        quantization_config=quantization_config if training_args.use_lora and lora_args.q_lora else None,
        **model_load_kwargs,
    )
    ...
    trainer = Trainer(
        model=model, tokenizer=tokenizer, args=training_args, **data_module, callbacks=[WandbLoggingCallback()] 
    )
...
class WandbLoggingCallback(TrainerCallback):
    def on_step_end(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        # Check if log_history has any entries
        if state.log_history and "loss" in state.log_history[-1]:
            loss = state.log_history[-1]["loss"]
            wandb.log({"Train Loss": loss, "Step": state.global_step})
        
    def on_evaluate(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        # Check if the last entry in log_history contains evaluation metrics
